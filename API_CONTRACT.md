...
The packaged `api.json` is generated by the `npm run build` that runs right
before `npm publish`. That build does not receive `RELEASE_TAG` or
`RELEASE_PUBLISHED_AT`, which only the CDN workflow sets, so its `repository`
field points to the repository root instead of the release tag and
`generatedAt` is the build time. Both copies come from
