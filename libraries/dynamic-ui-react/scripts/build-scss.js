...
files.forEach((file) => {
  ...
  execSync(`sass --load-path=node_modules --no-source-map ${file} > ${rawFilePath}`);
  execSync(`sass --load-path=node_modules --no-source-map --style=compressed ${file} > ${minFilePath}`);
